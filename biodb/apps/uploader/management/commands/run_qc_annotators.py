...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not all_annotators:
            self.stdout.write(self.style.WARNING("No annotators exist to annotate."))
            return
        ...
        try:
            ...
            raise CommandError(
                f"An error occurred when trying to retrieve all entries from the" f" {ArrayData.__name__} table."
            )

        if not all_data:
            self.stdout.write(self.style.WARNING("No ArrayData exists to annotate."))
            return
        ...
        self.stdout.write(
            f"There are {n_annotators} annotators and {n_data} entries in the"
